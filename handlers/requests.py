    ...
    try:
        ...
        if 0 <= request_index < len(user_requests):
            ...
            await query.message.edit_text(details_message,
                                          reply_markup=keyboards.back_btn(start_date,
...
async def process_supply_type_selection(query: types.CallbackQuery, state: FSMContext):
    ...
    logger.debug(f"Selected supply types: {selected_supply_types}")
...
async def process_create_notification(query: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.debug(f"Box IDs to save: {box_id}")  # Логирование перед сохранением

        await redis_client.save_request(
            user_id=query.from_user.id,
            warehouse_ids=warehouse_ids,
            boxTypeID=','.join(box_id),  # Передаем boxTypeID как строку
        ...
        await query.message.answer("Произшла ошибка при сохранении вашего запроса. Пожалуйста, попробуйте позже.")
